...
, m_bChartRun(false)				// Chart Run Flag
...
void CUDT_IRDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	*/
	...
	DDX_Control(pDX, IDC_BTN_RUN, m_btnchartRun);
}
...
BEGIN_MESSAGE_MAP(CUDT_IRDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BTN_RUN, &CUDT_IRDlg::OnBnClickedBtnChartRun)
...
void CUDT_IRDlg::OnBnClickedBtnStart()
{
	...
	else	// End
	{
		...
		m_editFilename.SetWindowTextW(_T(""));
	}
}
		...
		if (m_bStart)
		{
			...
			if (m_nMode == 8) // active mode
			{
				if (m_pMarkers[PELVIS].Z)
				{
					...
					{
						m_fPreviousSpeed = m_fSpeed;
						float calculatedSpeed = (float)(m_fK1 * m_fPelAcc + (m_fK2 - 1) * m_fPelSpd + m_fK3 * (m_fPelPos - m_fXrpos)) + Vw;
						calculatedSpeed = calculatedSpeed + 4.5;

						if (abs(calculatedSpeed - m_fPreviousSpeed)>0.1)
						{
							body_detection = 0;
							m_fSpeed = m_fPreviousSpeed;
							PrintStr(&CString(_T("Error : Pelvis Position is disturbed")));
						}
						else{
							body_detection = 1;
							m_fSpeed = calculatedSpeed;
						}

						char buffer[1024];

						sprintf_s(buffer, "%.2f", m_fK1 * m_fPelAcc);
						m_ValueA1.SetWindowText(CString(buffer));

						sprintf_s(buffer, "%.2f", (m_fK2 - 1) * m_fPelSpd);
						m_ValueA2.SetWindowText(CString(buffer));

						sprintf_s(buffer, "%.2f", m_fK3 * (m_fPelPos - m_fXrpos));
						m_ValueA3.SetWindowText(CString(buffer));

						sprintf_s(buffer, "%.2f", Vw);
			...
			if (m_pVRClientSocket)
			{
				...
				sprintf_s(strSend2, "%d %.3f %.2f %d %.2f %.3f %d\n", 7, m_fSpeed, m_fComfSpd, m_fRepeatTime, m_fSteady_Time_Interval, global_current_time, body_detection);
				m_pVRClientSocket->Send(strSend2, static_cast<int>(strlen(strSend2)));
			}
...
void CUDT_IRDlg::getData()
{
	...
	do
	{
		...
		if (m_currentIndex < sampleSize)
		{
			...
			m_dataSeriesA8[m_currentIndex] = dataA8;
		...
		else
		{
			...
			shiftData(m_dataSeriesA8, sampleSize, dataA8);
	...
	{
		DataSave_FSR(global_current_time, m_dataSeriesA1[m_currentIndex - 1], m_dataSeriesA2[m_currentIndex - 1], m_dataSeriesA3[m_currentIndex - 1], m_dataSeriesA4[m_currentIndex - 1], m_dataSeriesA5[m_currentIndex - 1], m_dataSeriesA6[m_currentIndex - 1], m_dataSeriesA7[m_currentIndex - 1], m_dataSeriesA8[m_currentIndex - 1]);
...
void CUDT_IRDlg::DataSave(float fTime)
{
	if (m_hFile == NULL && m_bStart)
	{
		...
		m_editFilename.GetWindowTextW(editFilename);

		if (editFilename.IsEmpty())
		{
			SYSTEMTIME time;
			GetLocalTime(&time);
			Filename.Format(_T("%d-%02d-%02d_h%02dm%02ds%02d"), time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute, time.wSecond);
			if (m_nMode == 7)
				Filename = Filename + _T("_passive.txt");
			else if (m_nMode == 8)
				Filename = Filename + _T("_active.txt");
		}
		else{
			SYSTEMTIME time;
			GetLocalTime(&time);
			Filename.Format(_T("%d-%02d-%02d_h%02dm%02ds%02d_"), time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute, time.wSecond);
			if (m_nMode == 7)
				Filename = Filename + editFilename + _T("_passive.txt");
			else if (m_nMode == 8)
				Filename = Filename + editFilename + _T("_active.txt");
		}
...
void CUDT_IRDlg::DataSave_FSR(float fTime, double FSR1, double FSR2, double FSR3, double FSR4, double FSR5, double FSR6, double trigger, double errorComm)
{
	if (m_hFile_FSR == NULL)
	{
		CString Filename;
		...
		if (editFilename.IsEmpty())
		{
			...
			Filename.Format(_T("%d-%02d-%02d_h%02dm%02ds%02d"), time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute, time.wSecond);
			if (m_nMode == 7)
				Filename = Filename + _T("_passive_FSR.txt");
			else if (m_nMode == 8)
				Filename = Filename + _T("_active_FSR.txt");
		}
		else{
			SYSTEMTIME time;
			GetLocalTime(&time);
			Filename.Format(_T("%d-%02d-%02d_h%02dm%02ds%02d_"), time.wYear, time.wMonth, time.wDay, time.wHour, time.wMinute, time.wSecond);
			if (m_nMode == 7)
				Filename = Filename + editFilename + _T("_passive_FSR.txt");
			else if (m_nMode == 8)
				Filename = Filename + editFilename + _T("_active_FSR.txt");
		}
		...
		char strData[73] = { 0 };
		sprintf_s(strData, "%% Time\tA1\tA2\tA3\tA4\tA5\tA6\tTrigger\tSpeed\terrorComm\t");
	...
	else if (m_nMode == 7 || m_nMode == 8)
	{
		...
		char str[100] = { 0, };
		sprintf_s(str, "%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t", FSR1, FSR2, FSR3, FSR4 - 300, FSR5 - 300, FSR6 - 300, trigger, m_fSpeed, errorComm);
...
void CUDT_IRDlg::OnBnClickedBtnChartRun()
{
	if (!m_bChartRun)
	{
		m_btnchartRun.SetWindowTextW(_T("Close"));
		drawChart(&m_ChartViewer);
		m_bChartRun = true;
	}
	else
	{
		m_btnchartRun.SetWindowTextW(_T("Run"));
		m_bChartRun = false;
	}
}